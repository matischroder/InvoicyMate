...
interface ParsedCommand {
    action: 'create_invoice' | 'list_clients' | 'add_client' | 'list_invoices' | 'help';
...
export class BotAIService {
    ...
    private async parseUserIntent(message: string): Promise<ParsedCommand> {
        ...
        // Agregar cliente
        if (lowerMessage.includes('agregar cliente') || lowerMessage.includes('add client')) {
        ...
        // Ver facturas
        if (lowerMessage.includes('ver facturas') || lowerMessage.includes('mostrar facturas')) {
            return {
    ...
    private async executeCommand(userId: number, command: ParsedCommand): Promise<string> {
    ...
    private extractClientInfo(message: string): Partial<CreateClientRequest> {
        ...
        const nameMatch = message.match(/(?:agregar cliente|add client)\s+(.+?)(?:\s+abn|\s*$)/i);
    ...
    private formatClientSelection(clients: ClientWithInvoiceCount[]): string {
        return 'Selecciona un cliente para la factura:\n\n' + 
